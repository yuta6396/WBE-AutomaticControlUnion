...
# PSOアルゴリズムの実装
def PSO_LDIWM(objective_function, bounds, num_particles, num_iterations, w_max, w_min, c1, c2):
    ...
    return global_best_position,  result_value, position_history
...
def genetic_algorithm(pop_size, gene_length, num_generations, crossover_rate,
    ...
    for generation in range(num_generations):
        fitness = calculate_fitness(population, black_box_function)

        current_best_fitness = np.min(fitness)
        current_best_individual = population[np.argmin(fitness)]

        if current_best_fitness < best_fitness:
            f.write(f"{current_best_fitness=},  {best_fitness=}")
    ...
    f.write(f"\n{gene_history=}")
    return best_fitness, best_individual, gene_history
