...
resistance_low = 0
resistance_high = 0.1
input_low = resistance_low
input_high = resistance_high
num_divisions=2
Zgrid_bottm = 0
Zgrid_width = 3
Ygrid_left= 15
...
dirname = f"test_result/GS_1grid/{Opt_purpose}_{input_var}={input_low}-{input_high}_Z={Zgrid_bottm}-{Zgrid_bottm+Zgrid_width-1}_Y={Ygrid_left}-{Ygrid_left+Ygrid_width-1}_{current_time}"
...
def update_netcdf(init: str, output: str, pe: int, Zgrid, Ygrid, input_value):
    """NetCDFファイルの変数を更新する"""
...
def sim(Zgrid, Ygrid, input_value):
    """
    ...
    for pe in range(nofpe):
        ...
        init = update_netcdf(init, output, pe, Zgrid, Ygrid, input_value)
...
def grid_search(objective_function, f, dirname):
    ...
    best_params = None
    results = np.zeros((Zgrid_width, Ygrid_width, num_divisions))
    # カラーマップを取得
    cmap = get_cmap("viridis")  # 好きなカラーマップ（例: 'viridis'）
    colors = cmap(np.linspace(0, 1, Ygrid_width))  # Ygrid_width種類の色を生成

    for z_i in range(Zgrid_bottm, Zgrid_bottm+Zgrid_width):
        best_score = float('inf')
        best_params = None
        f.write(f"\n\nZ={z_i}\n")
        for y_i in range(Ygrid_left, Ygrid_left+Ygrid_width):
            cnt = 0
            for input_i in np.linspace(input_low, input_high, num_divisions):
                print(f"z={z_i}, y={y_i}: input_size={input_i}")
                score = objective_function(z_i, y_i, input_i)
                print(score)
                results[z_i-Zgrid_bottm][y_i-Ygrid_left][cnt] = score
                if score < best_score:
                    best_score = score
                    best_params = [y_i, cnt]
                    f.write(f"{best_params=}: best_score={best_score}\n")
                cnt +=1

        plt.figure(figsize=(8, 6))
        for y_i in range(Ygrid_width):
            plt.plot(np.linspace(input_low, input_high, num_divisions), results[z_i-Zgrid_bottm][y_i], marker='o',label=f"Y={y_i+Ygrid_left}", color=colors[y_i], lw=2, ms=3)
        plt.xlabel('Input value', fontsize=20)
        plt.ylabel('Accumulated precipitation (%)', fontsize=20)
        plt.title(f"Y={Ygrid_left}-{Ygrid_left+Ygrid_width-1}, Z={z_i}", fontsize=20)
        plt.legend()
        plt.grid(True)
        plt.savefig(f"{dirname}/LineGraph_Z={z_i}.png", dpi = 300)
